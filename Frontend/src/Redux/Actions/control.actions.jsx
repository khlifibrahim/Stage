...
export const setExecutedAt = () => async (dispatch, getState) => {
    const { control } = getState();
    if(!control.executedAt.executed) {
        const currentDate = new Date();
        const at = `${currentDate.getDate()}-${currentDate.getMonth() + 1}-${currentDate.getFullYear()} ${currentDate.getHours()}:${currentDate.getMinutes()}:${currentDate.getSeconds()}`;
        
        dispatch({
            type: SET_EXECUTED_AT,
            payload: { executed: true, at: at}
        })
    }
}
